...
function filopodiaLength = ComputeFilopodiaLength(iXPath, iYPath)

    iXPath = double(iXPath);
    iYPath = double(iYPath);

    filopodiaLength = 0;
    for i=2:(length(iXPath))
        filopodiaLength = filopodiaLength + norm([iXPath(i), iYPath(i)] - [iXPath(i-1), iYPath(i-1)]);
        %filopodiaLength = filopodiaLength + sqrt(abs(iXPath(i) - iXPath(i-1)) + abs(iYPath(i) - iYPath(i-1)));
    end
end
