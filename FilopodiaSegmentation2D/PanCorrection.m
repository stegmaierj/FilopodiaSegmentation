...
function PanCorrection(object, eventdata)
    
    %% get the limits of the current axes
    global settings;

    settings.axisRange = [get(gca, 'XLim'), get(gca, 'YLim')];
    settings.disablePan = true;
    
    t = timer('TimerFcn', 'pan off; updateVisualization;', 'StartDelay',0.01);
    start(t)
end