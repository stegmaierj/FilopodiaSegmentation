function varargout = livewire(I, dRadius, dCaptureRadius, figureHandle, rawImage, xLim, yLim)
...
if nargin < 2
    ...
    SOptions.dRadius = dRadius;
    SOptions.dCaptureRadius = dCaptureRadius;
end
% -------------------------------------------------------------------------
...
figure(hF);
try
    oldButtonDownCallback = get(hF, 'WindowButtonDownFcn');
...
%             set([hLine1, hLine2], 'XData', dXData, 'YData', dYData);
%             drawnow expose
            set(hF, 'WindowButtonMotionFcn', '', 'WindowButtonDownFcn', oldButtonDownCallback, 'KeyPressFcn', '');
            lRegularEnd = true;
            uiresume(hF);
        end
